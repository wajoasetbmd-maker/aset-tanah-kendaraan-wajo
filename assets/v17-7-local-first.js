/* ASET TANAH DAN KENDARAAN WEB V17.7 — local-first + GPS presisi + surat cepat */
(() => {
  'use strict';
  ...
  async function remapLetterId(oldId, newId, result={}) {
    ...
    await new Promise((resolve, reject) => {
      ...
      store.delete(`${userScope()}:${oldId}`);
      const updated = {...letter, ...result, id:newId, pending:false, local:false};
  ...
  async function syncOutbox() {
    ...
    const operations = (await getOutbox()).slice(0, 100);
    if (!operations.length) { updateStatus('Lokal siap', 'ready'); return; }
    syncRunning = true;
    updateStatus(`Menyinkronkan ${operations.length} perubahan`, 'syncing');
    try {
      ...
      const ackIds = new Set((result.ack || []).map(item => Number(item.localQueueId)));
      await deleteOutboxIds([...ackIds]);
      for (const ack of result.ack || []) {
        const operation = String(ack.operation || '').toUpperCase();
        if (operation === 'LETTER' && ack.serverId) {
          await remapLetterId(String(ack.recordId || ''), String(ack.serverId), ack.result || {});
        } else if (operation === 'UPSERT' || operation === 'DELETE') {
          await markRecordSynced(moduleName(), String(ack.recordId || ''), String(ack.serverId || ack.recordId || ''), ack.result || {}, operation);
        }
      }
      const failed = result.failed || [];
      if (failed.length) updateStatus(`${failed.length} perubahan gagal`, 'error');
      else updateStatus('Tersinkron', 'ready');
      if ((await getOutbox()).length) setTimeout(syncOutbox, 1200);
    } catch (error) {
      console.warn('Sinkronisasi web local-first gagal:', error);
      updateStatus('Sinkron tertunda', 'pending');
    } finally {
      syncRunning = false;
    }
  }
  ...
  async function saveLetterLocal(payload) {
    ...
    const officer = officerList().find(item => String(item.id) === String(payload.officerId)) || {};
    const letter = {
  ...
  function officerList() {
    ...
    return b.officers || b.officersAll || [];
  }
  ...
  async function webServer(fn, ...args) {
    ...
    mirrorCloudResult(fn, args, result).catch(console.warn);
    return result;
  }
  ...
  async function downloadSnapshot(force=false) {
    ...
    try {
      ...
      while (cursor) {
        ...
        updateStatus(`Cache ${received}/${Number(info.totalRows || received)}`, 'syncing');
  ...
  function ensureStatusNode() {
    ...
    if (!node) {
      ...
      node.title = 'Klik untuk memeriksa pembaruan data';
      node.onclick = () => { syncOutbox(); refreshIfChanged(true); };
  ...
  async function renderLetterPreview(payload, previewWindow) {
    ...
    const officer = officers.find(item => String(item.id) === String(payload.officerId)) || {};
    const vehicle = payload.vehicle || {};
    ...
    const map = {
      ...
      ALAMAT_1:settings.ALAMAT_1 || '', ALAMAT_2:settings.ALAMAT_2 || '',
  ...
  async function localPreviewById(id, windowRef) {
    const letter = await getLocalLetter(id);
  ...
  window.WebLocalFirstV177 = {version:VERSION, syncNow:syncOutbox, refresh:() => refreshIfChanged(true), download:() => downloadSnapshot(true)};
})();
