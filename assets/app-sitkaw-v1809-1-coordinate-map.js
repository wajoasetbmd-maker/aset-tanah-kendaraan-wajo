...
(() => {
  'use strict';
  const VERSION = 'WEB V18.0.9.1';
  const PATCH_KEY = '__sitkawCoordinateWebV18091';
  ...
  function updateCoordinateLabels() {
    ...
    document.querySelectorAll('[data-land-filter]').forEach(card => {
      ...
      if (value === 'SUDAH DIPLOTING' && label && label.textContent !== 'Ada Koordinat') label.textContent = 'Ada Koordinat';
      if (value === 'BELUM DIPLOTING' && label && label.textContent !== 'Belum Ada Koordinat') label.textContent = 'Belum Ada Koordinat';
    });
    const select = document.getElementById('landFilter');
    if (select) Array.from(select.options).forEach(option => {
      if (String(option.value).toUpperCase() === 'SUDAH DIPLOTING' && option.textContent !== 'Ada Koordinat') option.textContent = 'Ada Koordinat';
      if (String(option.value).toUpperCase() === 'BELUM DIPLOTING' && option.textContent !== 'Belum Ada Koordinat') option.textContent = 'Belum Ada Koordinat';
    });
    document.querySelectorAll('.panel-head h3').forEach(title => {
      if (/Peta Koordinat Aset Tanah|Peta\s*&\s*Ploting|Peta Aset Tanah/i.test(title.textContent || '') && title.textContent !== 'Peta Lokasi Aset Tanah') title.textContent = 'Peta Lokasi Aset Tanah';
    });
    const map = document.getElementById('landMap');
    if (map) {
      const note = map.parentElement?.querySelector('.muted');
      if (note) { const next = `${mapCount} aset memiliki koordinat dan ditampilkan sebagai titik lokasi. Klik titik untuk melihat data atau membuka navigasi.`; if (note.textContent !== next) note.textContent = next; }
    }
  }
  ...
  function installPatch() {
    ...
    updateCoordinateLabels();
    bindClickableLandRows();
    // V18.0.9 used a document-wide MutationObserver. It repeatedly reacted to
    // its own label changes and could consume the browser main thread during
    // login/cache rendering. Known render functions are wrapped above, so a
    // global observer is unnecessary. One delayed refresh handles late paint.
    [120, 500].forEach(delay => setTimeout(() => {
      removePlottingMenu();
      updateCoordinateLabels();
      bindClickableLandRows();
    }, delay));
