/* ASET TANAH DAN KENDARAAN WEB V17.8 — local-first, sinkronisasi rinci, pejabat snapshot, surat cepat */
(() => {
  'use strict';
  ...
  async function queueOperation(operation, recordIdValue, payload) {
    const scope = userScope();
    await idb('outbox', 'readwrite', store => store.add({scope, module:moduleName(), operation, recordId:String(recordIdValue || ''), payload, createdAt:Date.now(), attempts:0}));
    updateStatus('Menunggu sinkronisasi', 'pending');
    if (navigator.onLine) setTimeout(syncOutbox, 250);
  }
  ...
  async function webServer(fn, ...args) {
    const localReadFns = new Set(['getDashboard','listKendaraan','listDashboardVehicles','getKendaraan','listTanah','getTanah','getLetterForm','getSuratForm','listSurat','listPejabat','getSettings','listUsers','listTanahUsers']);
    const localWriteFns = new Set(['saveKendaraan','deleteKendaraan','saveTanah','deleteTanah','saveSurat']);

    if (localWriteFns.has(fn) && window.state?.token) {
      if (fn === 'saveKendaraan') return await saveLocalRecord('KENDARAAN', args[1] || {});
      if (fn === 'deleteKendaraan') return await deleteLocalRecord('KENDARAAN', String(args[1] || ''));
      if (fn === 'saveTanah') return await saveLocalRecord('TANAH', args[1] || {});
      if (fn === 'deleteTanah') return await deleteLocalRecord('TANAH', String(args[1] || ''));
      if (fn === 'saveSurat') return await saveLetterLocal(args[1] || {});
    }

    if (fn === 'uploadLampiran' || fn === 'uploadDokumenTanah') {
      const payload = fn === 'uploadLampiran' ? (args[1] || {}) : {recordId:args[1], kind:args[2], ...(args[3] || {})};
      const recordIdValue = fn === 'uploadLampiran' ? payload.vehicleId : args[1];
      const attachmentPayload = fn === 'uploadLampiran'
        ? {kind:payload.type,fileName:payload.fileName,mimeType:payload.mimeType,base64:payload.base64}
        : {kind:args[2],fileName:payload.name,mimeType:payload.mimeType,base64:payload.base64};
      await queueOperation('ATTACHMENT', recordIdValue, attachmentPayload);
    ...
    if (localReadFns.has(fn) && await hasSnapshot()) {
      try {
        ...
        if (local != null && (!(Array.isArray(local)) || local.length || ['listPejabat','listUsers'].includes(fn))) {
          if (navigator.onLine) setTimeout(() => refreshIfChanged(false), 300);
          return local;
        }
      } catch (error) { console.warn('Baca lokal gagal, memakai server:', error); }
    }
    ...
    if (fn === 'saveSettings' || fn === 'saveTanahSettings') {
      const settings = await cloudServer('getSettings', state.token).catch(() => result || {});
  ...
  async function downloadLettersSnapshot() {
    if (moduleName() !== 'KENDARAAN' || !['ADMIN','OPERATOR'].includes(String(window.state?.user?.role || ''))) return;
    try {
      const first = await cloudServer('listSurat', state.token, {page:1,pageSize:75,search:''});
      await mirrorCloudResult('listSurat', [state.token,{page:1,pageSize:75,search:''}], first);
      const pages = Math.max(1, Number(first.pages || 1));
      for (let page=2; page<=pages; page++) {
        const result = await cloudServer('listSurat', state.token, {page,pageSize:75,search:''});
        await mirrorCloudResult('listSurat', [state.token,{page,pageSize:75,search:''}], result);
        await sleep(10);
      }
      await putMeta(key(userScope(), 'lettersReady'), true);
    } catch (error) { console.warn('Riwayat surat belum selesai dicache:', error); }
  }
  ...
  async function downloadSnapshot(force=false) {
    ...
    try {
      ...
      await downloadLettersSnapshot();
      const revision = await cloudServer('getOfflineRevisionV183', state.token);
      await putMeta(key(userScope(), 'revision'), revision);
      await putMeta(key(userScope(), 'snapshotReady'), true);
      await putMeta(key(userScope(), 'snapshotAt'), Date.now());
      updateStatus('Data lokal siap', 'ready');
      reserveNumbersIfNeeded();
    } catch (error) {
      console.warn('Download cache web gagal:', error);
      updateStatus('Cache belum lengkap', 'error');
    } finally {
  ...
  async function refreshIfChanged(force=false) {
    ...
    try {
      ...
      if (force || !local || String(local.revision) !== String(remote.revision)) await downloadSnapshot(true);
      else updateStatus('Data lokal terbaru', 'ready');
    } catch (error) { console.warn('Pemeriksaan revisi web gagal:', error); }
  }

  function ensureSyncPanel() {
    let panel = document.getElementById('webSyncDetailPanel');
    if (panel) return panel;
    panel = document.createElement('div');
    panel.id = 'webSyncDetailPanel';
    panel.style.cssText = 'display:none;position:fixed;right:18px;top:64px;z-index:10050;width:min(390px,calc(100vw - 24px));background:#fff;border:1px solid #ccd7e2;border-radius:14px;box-shadow:0 16px 42px rgba(22,48,75,.25);padding:14px;color:#23384d;font-family:Arial,sans-serif';
    panel.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;gap:10px"><b>Proses Sinkronisasi</b><button id="webSyncClose" type="button" style="border:0;background:#eef3f7;border-radius:50%;width:30px;height:30px;cursor:pointer">×</button></div><div id="webSyncSummary" style="margin-top:10px;font-size:13px"></div><div style="height:8px;background:#e8eef4;border-radius:999px;overflow:hidden;margin-top:9px"><div id="webSyncBar" style="height:100%;width:0;background:#2c6fb2;transition:width .2s"></div></div><div id="webSyncCurrent" style="font-size:12px;color:#53697d;margin-top:8px;word-break:break-word"></div><div id="webSyncPending" style="font-size:12px;max-height:150px;overflow:auto;margin-top:8px"></div><div id="webSyncError" style="display:none;margin-top:8px;padding:8px;border-radius:8px;background:#fde8ea;color:#8e202b;font-size:12px"></div><div style="display:flex;gap:8px;margin-top:12px"><button id="webSyncNow" type="button" style="flex:1;border:0;border-radius:9px;background:#1f5f99;color:#fff;padding:9px;font-weight:700;cursor:pointer">Sinkronkan Sekarang</button><button id="webRefreshNow" type="button" style="flex:1;border:1px solid #bccbd8;border-radius:9px;background:#fff;color:#2f4b63;padding:9px;font-weight:700;cursor:pointer">Cek Data Server</button></div>`;
    document.body.appendChild(panel);
    panel.querySelector('#webSyncClose').onclick = () => { panel.style.display='none'; };
    panel.querySelector('#webSyncNow').onclick = () => syncOutbox();
    panel.querySelector('#webRefreshNow').onclick = () => refreshIfChanged(true);
    return panel;
  }
  ...
  async function refreshSyncPanel() {
    ...
    const total = Math.max(syncView.total || 0, syncView.done + pending.length, pending.length);
    const percent = syncView.running ? Math.max(0, Math.min(99, syncView.percent || 0)) : (pending.length ? Math.max(0, Math.min(99, syncView.percent || 0)) : 100);
    panel.querySelector('#webSyncSummary').innerHTML = `<b>${syncView.running ? 'Sedang berjalan' : (pending.length ? 'Menunggu sinkronisasi' : 'Semua data tersinkron')}</b><br>${syncView.done || 0} selesai · ${pending.length} menunggu · ${percent}%`;
    panel.querySelector('#webSyncBar').style.width = `${percent}%`;
    panel.querySelector('#webSyncCurrent').textContent = syncView.current ? `Saat ini: ${syncView.current}` : (syncView.lastSyncAt ? `Sinkron terakhir: ${new Date(syncView.lastSyncAt).toLocaleString('id-ID')}` : 'Belum ada proses sinkronisasi.');
    panel.querySelector('#webSyncPending').innerHTML = pending.length ? `<b>Antrean:</b>${pending.slice(0,8).map((item,i)=>`<div style="padding:5px 0;border-bottom:1px solid #edf1f4">${i+1}. ${escText(operationLabel(item))}</div>`).join('')}${pending.length>8?`<div style="padding-top:5px">+ ${pending.length-8} data lainnya</div>`:''}` : '';
    const error = syncView.lastError || (syncView.failed[0]?.error || '');
    const errorNode = panel.querySelector('#webSyncError');
    errorNode.style.display = error ? 'block' : 'none';
    errorNode.textContent = error ? `Kendala: ${error}` : '';
  }
  ...
  function ensureStatusNode() {
    ...
    if (!node) {
      ...
      node.title = 'Klik untuk melihat persen, antrean, nama data, dan kendala sinkronisasi';
      node.onclick = async () => { const panel=ensureSyncPanel(); panel.style.display=panel.style.display==='none'?'block':'none'; await refreshSyncPanel(); };
  ...
  window.submitLetter = async function submitLetterV177() {
    ...
    if (!previewWindow) { toast('Popup diblokir. Izinkan popup untuk melihat surat.','error'); return; }
    ...
    const selectedOfficer = resolveOfficer({officerId:payload.officerId});
    payload.officerSnapshot = selectedOfficer;
    payload.officerName = selectedOfficer.name;
    payload.officerNip = selectedOfficer.nip;
    payload.officerPosition = selectedOfficer.position;
    if (typeof loading === 'function') loading(true,'Membuka surat dari template lokal...',25,'Penyimpanan server berjalan di latar belakang.');
    try {
      await renderLetterPreview(payload, previewWindow);
      const result = await saveLetterLocal(payload);
      await consumeReservedNumber(payload.letterNumber);
      for (const [kind,file] of uploadFiles.filter(x => x[1])) {
        const data = await readFile(file);
        await queueOperation('ATTACHMENT', payload.vehicleId, {kind,fileName:file.name,mimeType:file.type,base64:data});
      }
      closeModal();
      toast(`Surat ${result.number} dibuat cepat dari template lokal. Sinkronisasi ke server berjalan di latar belakang.`);
      if (navigator.onLine) setTimeout(syncOutbox, 300);
      if (window.state?.view === 'letters' && typeof fetchLetters === 'function') setTimeout(fetchLetters, 100);
    } catch (error) {
      previewWindow.close();
  ...
  function installHooks() {
    ...
    installed = true;
    webServer.__v177 = true;
    window.server = webServer;

    const originalHandleLogin = window.handleLogin;
    if (typeof originalHandleLogin === 'function') {
      window.handleLogin = async function handleLoginV177(event) {
        if (navigator.onLine !== false || String(window.state?.asset || '').toUpperCase() !== 'KENDARAAN') {
        ...
        try {
          ...
          const cached = JSON.parse(localStorage.getItem('v17_1_offline_session_KENDARAAN') || 'null');
          const data = new TextEncoder().encode(password);
          const digest = await crypto.subtle.digest('SHA-256', data);
          const passwordHash = Array.from(new Uint8Array(digest)).map(b => b.toString(16).padStart(2,'0')).join('');
          if (!cached || String(cached.username || '').toLowerCase() !== username.toLowerCase() || cached.passwordHash !== passwordHash) {
            throw new Error('Login offline belum tersedia untuk akun ini. Lakukan login online satu kali pada perangkat ini.');
          }
          ...
          document.getElementById('userMiniName').textContent = state.user.name || state.user.username;
          document.getElementById('userMiniRole').textContent = state.user.role;
          document.getElementById('topUser').textContent = `KENDARAAN · ${state.user.name || state.user.username} · ${state.user.role} · OFFLINE`;
          renderVehicleShell();
          await navigate('dashboard');
          updateStatus('Offline · memakai data lokal', 'pending');
        } catch (error) {
          if (errorBox) {
    ...
    if (typeof originalStartApp === 'function') {
      window.startApp = async function startAppV177() {
        const result = await originalStartApp.apply(this, arguments);
        ensureStatusNode();
        updateStatus('Menyiapkan data lokal', 'syncing');
        try {
          ...
          if (await hasSnapshot()) updateStatus('Data lokal siap', 'ready');
          setTimeout(() => downloadSnapshot(false), 300);
          setTimeout(() => refreshIfChanged(false), 1500);
          setTimeout(() => reserveNumbersIfNeeded(), 700);
    ...
    originalPreviewLetterPdf = window.previewLetterPdf;
    if (typeof originalPreviewLetterPdf === 'function') {
      window.previewLetterPdf = async function previewLetterPdfV177(id, existingWindow=null) {
        const w = existingWindow || window.open('about:blank','_blank');
        if (!w) { toast('Popup diblokir browser.','error'); return; }
        try {
          if (await localPreviewById(id, w)) return;
        } catch (error) { console.warn(error); }
        return originalPreviewLetterPdf(id, w);
      };
    }

    window.addEventListener('online', () => { updateStatus('Online · sinkronisasi', 'syncing'); syncOutbox(); refreshIfChanged(false); });
    window.addEventListener('offline', () => updateStatus('Offline · memakai data lokal', 'pending'));
    setInterval(() => { if (window.state?.token) { syncOutbox(); refreshIfChanged(false); } }, 5 * 60 * 1000);
  }

  document.addEventListener('DOMContentLoaded', installHooks, {once:true});
  if (document.readyState !== 'loading') installHooks();
  window.WebLocalFirstV178 = {version:VERSION, syncNow:syncOutbox, refresh:() => refreshIfChanged(true), download:() => downloadSnapshot(true), showSync:async()=>{const p=ensureSyncPanel();p.style.display='block';await refreshSyncPanel();}};
})();
